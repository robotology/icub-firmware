...
static uint16_t s_eocfg_eps_eb1_ep2index(eOnvEP8_t ep);

// endpoint mn
static void s_eocfg_eps_eb1_mn_ram_initialise(void *ram);

// endpoint mc
static void s_eocfg_eps_eb1_mc_ram_initialise(void *ram);

// endpoint as
static void s_eocfg_eps_eb1_as_ram_initialise(void *ram);
...
{  
    {   // management
        ...
        EO_INIT(.fptr_initialise)                   s_eocfg_eps_eb1_mn_ram_initialise
    },        
    ...
    {   // motion-control         
        ...
        EO_INIT(.fptr_initialise)                   s_eocfg_eps_eb1_mc_ram_initialise        
    },
    ...
    {   // analog-sensors         
        ...
        EO_INIT(.fptr_initialise)                   s_eocfg_eps_eb1_as_ram_initialise                
    }  
...
{
    EO_INIT(.boardnum)                  eocfg_EPs_eb1_boardnumber,
    EO_INIT(.dummy)                     {0, 0, 0},
...
extern void* eocfg_EPs_eb1_Get_RAM(eOnvEP8_t ep)
{
...
EO_VERIFYproposition(eocfg_eps_eb1_ep_mn_val, 0x01 == eoprot_endpoint_management);
EO_VERIFYproposition(eocfg_eps_eb1_ep_mc_val, 0x11 == eoprot_endpoint_motioncontrol);
EO_VERIFYproposition(eocfg_eps_eb1_ep_as_val, 0x21 == eoprot_endpoint_analogsensors);
EO_VERIFYproposition(eocfg_eps_eb1_ep_sk_val, 0x31 == eoprot_endpoint_skin);
...
static uint16_t s_eocfg_eps_eb1_ep2index(eOnvEP8_t ep)
{    
    if(ep > sizeof(s_eocfg_eps_eb1_hashtable))
    {
        return(EOK_uint16dummy);
    }
    
    uint8_t r = s_eocfg_eps_eb1_hashtable[ep];
...
static void s_eocfg_eps_eb1_mn_ram_initialise(void *ram)
{
    ...
    uint16_t i = s_eocfg_eps_eb1_ep2index(eoprot_endpoint_management);    
    if(EOK_uint16dummy != i)
    {   
...
{
    // 1. assign the ram to the local pointer, so that future calls to eocfg_nvsEP_eb1_Get_RAM() shall give the ram back
    uint16_t i = s_eocfg_eps_eb1_ep2index(eoprot_endpoint_motioncontrol);    
    if(EOK_uint16dummy != i)
    {   
        s_eocfg_eps_eb1_ram[i] = ram;
    }  
...
{
    // 1. assign the ram to the local pointer, so that future calls to eocfg_nvsEP_eb1_Get_RAM() shall give the ram back
    uint16_t i = s_eocfg_eps_eb1_ep2index(eoprot_endpoint_analogsensors);    
    if(EOK_uint16dummy != i)
    {   
        s_eocfg_eps_eb1_ram[i] = ram;
    }  
