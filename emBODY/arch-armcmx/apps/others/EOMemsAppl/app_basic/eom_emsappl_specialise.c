...
static void (*s_eom_emsappl_specialise_on_ropframe_received)(EOpacket*) = NULL;
...
extern const ipal_cfg_t* eom_emsappl_specialise_ipal_cfg = &ipal_cfg;


extern const eOerrman_cfg_t  eom_emsappl_specialise_errcfg = 
{
    .extfn.usr_on_error = s_OnError
};

extern const eOmipnet_cfg_dtgskt_t eom_emsappl_specialise_dtgskt_cfg = 
{   
    .numberofsockets            = 2, 
...
extern void eom_emsappl_specialise_updserver_start(void)
{
    s_sem_tx_pkt =  osal_semaphore_new(255, 0);

    s_host_connected = eobool_false;

    eo_errman_Assert(eo_errman_GetHandle(), (NULL != s_sem_tx_pkt), "emsappl_specialise", "semaphore is NULL");
...
extern void eom_emsappl_specialise_onpktreceived_set( void (*cbk)(EOpacket*)  )
{
    s_eom_emsappl_specialise_on_ropframe_received = cbk;
}

extern void eom_emsappl_specialise_otherthings(void)
{

    eom_emsappl_extra_transceiver_init();

}


extern void eom_emsappl_specialise_transmit(EOpacket *txpkt)
{
...
extern eObool_t eom_emsappl_specialise_connect(eOipv4addr_t remotehostaddr)
{
...
static void s_udpserver_startup(EOMtask *p, uint32_t t)
{
    ...
    //hal_trace_timing_entity_add(2, (void (*)(void))s_udpserver_run);
...
static void s_udpserver_run(EOMtask *p, uint32_t t)
{
    ...
    //hal_trace_timing_entity_switch_to(2); 
    ...
    if(s_event_from_skt_rops == (evt & s_event_from_skt_rops))
    {
        ...
        if((eores_OK == res) && (NULL != s_eom_emsappl_specialise_on_ropframe_received))
        {
            s_eom_emsappl_specialise_on_ropframe_received(s_rxpkt);
        }
...
//    }
    ...
    //hal_trace_timing_entity_switch_to_idle();
