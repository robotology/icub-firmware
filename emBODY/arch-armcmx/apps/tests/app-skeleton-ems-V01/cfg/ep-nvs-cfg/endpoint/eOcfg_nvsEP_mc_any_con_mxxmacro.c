...
#define OFFSETof_mconfig__upto02descrforchameleon06            (OFFSETafter_mconfig__maxcurrentofmotor)
#define CAPACITY_mconfig__upto02descrforchameleon06            (2*sizeof(eOutil_chameleon_descriptor_t))
extern EOnv_con_t MMACRO_NAMEOFVARIABLE(MMACRO_PSTR, MMACRO_MSTR, _mconfig__upto02descrforchameleon06) =
{   
    EO_INIT(.id)        MMACRO_GETNVID(MMACRO_EXTERNALPREFIX_GETID, _mconfig__upto02descrforchameleon06, MMACRO_MNUM),    
    EO_INIT(.capacity)  CAPACITY_mconfig__upto02descrforchameleon06,
    EO_INIT(.resetval)  (const void*)&eo_cfg_nvsEP_mc_any_con_mxxdefault_defaultvalue.mconfig.upto02descrforchameleon06,
    EO_INIT(.offset)    OFFSETof_mconfig__upto02descrforchameleon06,
    EO_INIT(.typ)       EO_nv_TYP(EOK_cfg_nvsEP_mc_any_con_mxx_NVFUNTYP_mconfig__upto02descrforchameleon06),
    EO_INIT(.fun)       EO_nv_FUN(EOK_cfg_nvsEP_mc_any_con_mxx_NVFUNTYP_mconfig__upto02descrforchameleon06)
};
#define OFFSETafter_mconfig__upto02descrforchameleon06          (OFFSETof_mconfig__upto02descrforchameleon06+CAPACITY_mconfig__upto02descrforchameleon06)


// guard on alignement of variables. if it doesnt compile then ... the compiler has surely inserted some holes
MMACRO_VERIFYSIZEOF(MMACRO_MNUM, eOmc_motor_config_t, OFFSETafter_mconfig__upto02descrforchameleon06-MMACRO_MOFF);


#define OFFSETof_mstatus                                        (OFFSETafter_mconfig)
#define CAPACITY_mstatus                                        sizeof(eOmc_motor_status_t)
extern EOnv_con_t MMACRO_NAMEOFVARIABLE(MMACRO_PSTR, MMACRO_MSTR, __mstatus) =
{   
    EO_INIT(.id)        MMACRO_GETNVID(MMACRO_EXTERNALPREFIX_GETID, __mstatus, MMACRO_MNUM),
    ...
    EO_INIT(.typ)       EO_nv_TYP(EOK_cfg_nvsEP_mc_any_con_mxx_NVFUNTYP__mstatus),
    EO_INIT(.fun)       EO_nv_FUN(EOK_cfg_nvsEP_mc_any_con_mxx_NVFUNTYP__mstatus)
};
#define OFFSETafter_mstatus                                     (OFFSETof_mstatus+CAPACITY_mstatus)


// guard on alignement of variables. if it doesnt compile then ... the compiler has surely inserted some holes
MMACRO_VERIFYSIZEOF(MMACRO_MNUM, eOmc_motor_t, OFFSETafter_mstatus-MMACRO_MOFF);
