...
static hal_eth_onframerxandtx_t s_onframerxtx =
{
     .frame_new                  = NULL,
     .frame_movetohigherlayer    = NULL,
//    .frame_alerthigherlayer     = NULL, 
    .frame_onrxalerthigherlayer = NULL,
    .frame_copytohigherlayer = NULL,
    .frame_copyfromhigherlayer = NULL
};
...
//    uint8_t phy_link_is_up[HAL_BRDCFG_ETH__PHY_DEVICE_NUM];
    ...
    hal_eth_onframerxandtx_t *func_onrxtx = NULL;
    ...
    if(NULL == cfg)
    {
        hal_base_hid_on_fatalerror(hal_fatalerror_incorrectparameter, "hal_eth_init() cfg is null!!!");
        return(hal_res_NOK_nullpointer);
    }

    //until ipal with tcpnet and ipal with lwip will use different hal, i use this trick in order to not change hal_eth.h
    func_onrxtx = (hal_eth_onframerxandtx_t*)cfg->onframerx;
    
    if((hal_int_priorityNONE == cfg->priority) || (NULL == func_onrxtx) || (NULL == func_onrxtx->frame_copytohigherlayer) || (NULL == func_onrxtx->frame_copyfromhigherlayer) || (NULL == cfg->macaddress))
    {
    ...
    {
        memcpy(&s_onframerxtx, func_onrxtx, sizeof(hal_eth_onframerxandtx_t));
...
#warning different prototype of hal_eth_sendframe
//extern hal_result_t hal_eth_sendframe(void* data)
...
//     if(NULL != s_onframerx.getframefromhigherlayer)
//     {
//         uint64_t timecurr = 0;//eov_sys_LifeTimeGet(eov_sys_GetHandle());
//         sprintf(str, "hal_eth_sendframe: time=%llu", (timecurr));
//         hal_trace_puts(str);
        ...
        Tx_Desc[j].Size = s_onframerxtx.frame_copyfromhigherlayer(data, dp_u8);
        if(0 == Tx_Desc[j].Size)
        {
...
//     else
//     {
//         hal_eth_frame_t *frame = (hal_eth_frame_t *)data;
...
//         sprintf(str, "hal_eth_isr ERR A");
//         hal_trace_puts(str);
      goto rel;
    }
    if ((Rx_Desc[i].Stat & DMA_RX_SEG_MASK) != DMA_RX_SEG_MASK) {
//         sprintf(str, "hal_eth_isr ERR B, stat = 0x%x", Rx_Desc[i].Stat);
//         hal_trace_puts(str);
        goto rel;
    }
    RxLen = ((Rx_Desc[i].Stat >> 16) & 0x3FFF) - 4;
    if (RxLen > ETH_MTU) {
//               sprintf(str, "hal_eth_isr ERR C");
//         hal_trace_puts(str);
      ...
      goto rel;
          ...
          timecurr = 0;//eov_sys_LifeTimeGet(eov_sys_GetHandle());
//     sprintf(str, "hal_eth_isr before copy: time=%d", ((uint32_t)timecurr-timeprev));
//     hal_trace_puts(str);
//     timeprev = timecurr;
    
//     sprintf(str, "hal_eth_isr before copy: time=%llu", (timecurr));
//     hal_trace_puts(str);
...
//REMOVED   !!!!
//     /* Flag 0x80000000 to skip sys_error() call when out of memory. */
//     frame = s_onframerxtx.frame_new(RxLen | 0x80000000);
...
//     size = (RxLen | 0x80000000);
//     size_u16 = size;
//     frame = s_onframerxtx.frame_new(size);
//     
//     sprintf(str, "hal_eth_isr RxLen=0x%x size=0x%x", RxLen, size);
//     hal_trace_puts(str);
//     /* if 'alloc_mem()' has failed, ignore this packet. */
//     if (frame != NULL) 
//     {
...
//REMOVED   !!!!        
//         //put_in_queue (frame);
//         s_onframerxtx.frame_movetohigherlayer(frame);
 ...
 //ADDED!!! call higher layer to copy received pkt
//        size = (RxLen | 0x80000000);
        sp = (U32 *)(Rx_Desc[i].Addr & ~3);
//         if(s_onframerx.frame_copytohigherlayer != NULL)
//         {
            sp_u8 = (uint8_t *)sp;
            if(hal_eth_lowLevelUsePacket_ptr != NULL)
            {
                hal_eth_lowLevelUsePacket_ptr(sp_u8, size);
            }
            s_onframerxtx.frame_copytohigherlayer(/*(U8 *)(Rx_Desc[i].Addr & ~3)*/sp_u8, RxLen);

//         }
//         else
//         {
//             frame = s_onframerx.frame_new(size);
//             /* if 'alloc_mem()' has failed, ignore this packet. */
//             if (frame != NULL)
//             {
//                 dp = (U32 *)&frame->datafirstbyte[0];
...
//         }
        ...
        if(NULL != s_onframerxtx.frame_onrxalerthigherlayer)
        {
//             timecurr = 0; //eov_sys_LifeTimeGet(eov_sys_GetHandle());
//             sprintf(str, "hal_eth_isr alerthigherlayer: time=%llu", (timecurr));
//             hal_trace_puts(str);
            s_onframerxtx.frame_onrxalerthigherlayer();
        }
...
// sprintf(str, "hal_eth_isr 2");
// hal_trace_puts(str);
  } while ((Rx_Desc[i].Stat & DMA_RX_OWN) == 0);
  RxBufIndex = i;
// sprintf(str, "hal_eth_isr 3");
// hal_trace_puts(str);
...
extern hal_result_t hal_eth_hid_setmem(const hal_cfg_t *cfg, uint32_t *memory)
{
    ...
    Rx_Desc = NULL;
    Tx_Desc = NULL;
    memset(&s_onframerxtx, 0, sizeof(s_onframerxtx));
    s_hal_mac = NULL;
...
static void rx_descr_init (void) {
  ...
  RxBufIndex = 0;
    char str[30];
//     sprintf(str, "rx dma buf num: %d", hal_base_hid_params.eth_dmarxbuffer_num);
//     hal_trace_puts(str);
