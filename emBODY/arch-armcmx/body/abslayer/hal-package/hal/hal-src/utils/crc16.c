...
/* @file       crc16.c
    @brief      This file contains implementation of crc16 using lut, as described by Sven Reifegerste
...
extern void crc16_table_get(uint16_t polymomial, uint16_t *crctable)
{
...
extern uint16_t crc16_compute(const uint16_t *crc16table, uint16_t initvalue, uint8_t *data, uint32_t len)
{
	...
	while(len--) 
    {
        crc = (crc << 8) ^ crc16table[ ((crc >> 8) & 0xff) ^ *data++];
    }
