...
// --------------------------------------------------------------------------------------------------------------------
extern void ipal_debug_print(char *message)
{
    char str[100];
    sprintf(str, message);
    //hal_trace_puts(str);
...
extern ipal_result_t ipal_base_initialise(const ipal_cfg_t *cfg, uint32_t *data04aligned)
{
    ...
    if(ipal_ipstack_iitmod_lwip != cfg->arch_ipstack)
    {
        ipal_base_hid_on_fatalerror(ipal_error_unsupportedbehaviour, "ipal_base_initialise(): unsupported tcp/ip stack");
        return(ipal_res_NOK_generic);
    }
    
    //if user config one or more unsupported services, this function call ipal_base_hid_on_fatalerror.
...
__weak extern void mydebug(const char *format, ...)
{;}
