...
/* @file       eOcfg_nvsEP_mc_upperleg_usrcbk_pippo.c
    @brief     This file keeps examples for ems / pc104 of the user-defined functions used for endpoint mc_upperleg by
...
// the ems uses either the left or the right uppeleg endpoint, thus on ems it is clear that the nv->loc is relevant to 
// whcih joint.
...
{
    eo_cfg_nvsEP_mc_upperleg_con_jointNumber_t  jxx = (eo_cfg_nvsEP_mc_upperleg_con_jointNumber_t) xx;
    ...
    ep = ep;                                                // the ep value helps discriminate if it is left or right (endpoint_mc_leftupperleg or endpoint_mc_rightupperleg)
...
{
    eo_cfg_nvsEP_mc_upperleg_con_motorNumber_t  mxx = (eo_cfg_nvsEP_mc_upperleg_con_motorNumber_t) xx;
    eObool_t theOwnershipIsLocal = (NULL == nv->rem) ? eobool_true : eobool_false;
    eOipv4addr_t ip = nv->ip;
    eOnvEP_t ep = nv->ep;
...
extern void eo_cfg_nvsEP_mc_upperleg_usr_hid_UPDT_Jxx_jstatus(uint16_t xx, const EOnv* nv, const eOabstime_t time, const uint32_t sign)
{
    ...
    if(endpoint_mc_leftupperleg == ep)
    {
        // its ems eb6
    }
    else if(endpoint_mc_rightupperleg == ep)
    {
        // it is ems eb8
    }
...
#if 0
// this function is common to all mc endpoint 
