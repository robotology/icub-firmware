...
static void s_appl_init(void)
{
    ...
    hal_eeprom_cfg_t ee_cfg = { .dummy = 0 };
    ...
    s_can_init(my_6sg_cfg.gen_ee_data.board_address);
...
//TODO: manca il controllo sulla saturazione!!!
    ...
    {
        frame.id = (CAN_MSG_CLASS_PERIODIC) | (my_6sg_cfg.gen_ee_data.board_address<<4) | (CAN_CMD_TORQUE_VECTOR) ;
    ...
    else if(tx_outMsg_torqueForceData_on ==  my_6sg_cfg.behaviour_cfg.tx_outMsg_mode) 
    {
        ...
        //send force values
        frame.id = (CAN_MSG_CLASS_PERIODIC) | (my_6sg_cfg.gen_ee_data.board_address<<4) | (CAN_CMD_FORCE_VECTOR) ;
