...
#define HAL_led_t2index(t)              ((uint8_t)((t)))
...
static hal_boolval_t s_hal_led_initted_is(hal_led_t led);
...
static hal_boolval_t s_hal_led_initted[hal_leds_num] = { hal_false };
...
extern hal_result_t hal_led_on(hal_led_t led)
{
    ...
    if(hal_false == s_hal_led_initted_is(led))
    {
        return(hal_res_NOK_generic);
    }
    ...
    return(hal_gpio_setval(gc->port, gc->pin, hal_brdcfg_led__value_on));
}
...
extern hal_result_t hal_led_off(hal_led_t led)
{
    ...
    return(hal_gpio_setval(gc->port, gc->pin, hal_brdcfg_led__value_off));
}
...
extern hal_result_t hal_led_toggle(hal_led_t led)
{
    ...
    if(hal_brdcfg_led__value_off == val)
    {
        val = hal_brdcfg_led__value_on;
    }
    else
    {
        val = hal_brdcfg_led__value_off;
    }
...
//    }
    ...
    memset(s_hal_led_initted, hal_false, sizeof(s_hal_led_initted));
    return(hal_res_OK);  
}
...
static void s_hal_led_initted_set(hal_led_t led)
{
    s_hal_led_initted[HAL_led_t2index(led)] = hal_true;
}

static hal_boolval_t s_hal_led_initted_is(hal_led_t led)
{
    return(s_hal_led_initted[HAL_led_t2index(led)]);
}
