...
typedef struct              // size is 16 bytes
{
    ...
    int16_t                 scale;          /**< scale factor for the gains (and what about the rest?)*/
    int16_t                 offset;         /**< limit of the output of the pid */
...
/** @typedef    typedef struct eOmc_joint_config_t
    ...
    @warning    This struct must be of fixed and size and multiple of 4.
 **/
typedef struct                  // size is: 16+16+16+16+4+4+2+2+2+2+1+3 = 84
{
    eOmc_PID_t                  pidpos;
    eOmc_PID_t                  pidvel;
    eOmc_PID_t                  pidtor;
    ...
    uint16_t                    velocityshiftfactor;    // of ??
...
} eOmc_joint_config_t;          EO_VERIFYsizeof(eOmc_joint_config_t, 84);
...
typedef struct                  // size is 84+8+36+12+1+1+1+1 = 144
{
    ...
    uint8_t                     filler01[1];
} eOmc_joint_t;                 EO_VERIFYsizeof(eOmc_joint_t, 144);
