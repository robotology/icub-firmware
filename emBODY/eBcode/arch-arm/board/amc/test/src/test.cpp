...
namespace test::can {
    ...
    {
        embot::os::EventThread* evtsk = reinterpret_cast<embot::os::EventThread*>(p);
        if(nullptr != evtsk)
        {
            evtsk->setEvent(TesterSingleton::evRXcanframe);
        }
        rxcount ++; 
    ...
    void init(embot::os::EventThread *eventhread)
    {
        // i init the can driver
        embot::hw::can::Config canconfig {};
        canconfig.rxcapacity = TesterSingleton::canRXcapacity;
        canconfig.txcapacity = TesterSingleton::canTXcapacity;
        canconfig.onrxframe = embot::core::Callback(can::alertonrxframe, eventhread); 
        canconfig.ontxframe = embot::core::Callback(can::alertontxframe, eventhread);
    ...
    void run()
    {
        ...
        if(embot::hw::resOK == embot::hw::can::get(can::bus, canframe, remaining))
        {
            embot::core::print("Entered in test::can::run()");
            ...
            switch(canframe.data[0])
            {
                ...
                case 0x00:
                {
                    ...
                    uint8_t data[8] {0};
                    data[0] = 0xAA;
                    embot::hw::can::Frame outputframe {};
                    outputframe.id = 0x551;
                    outputframe.size = 8;
                    for(uint8_t i = 0; i < 8; i++) {outputframe.data[i] = data[i];}
                    embot::hw::can::put(can::bus, {outputframe.id, outputframe.size, outputframe.data});       
                    embot::hw::can::transmit(can::bus);
                } break;
...
namespace test::any {
    ...
    void init(void *p)
    {
        eventhread = reinterpret_cast<embot::os::EventThread*>(p);
        test::can::init(eventhread);            
    }
...
void TesterSingleton::test_embot_hw_init(void *p)
{
    test::any::init(p);   
}


void TesterSingleton::test_embot_hw_tick()
{
    static size_t tt {0};
    tt++;    
...
void TesterSingleton::test_embot_hw_run()
{
    test::any::run();
}
