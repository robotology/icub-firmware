...
static volatile int32_t encoderConvFactor = 0;
...
#warning acemor: TODO read in here 

#if 0
 abbiamo preso encoderInit() e messo dentro encoderConfig() 
 ed abbiamo lasciato in encoderInit() solo poca roba.
 ed abbiamo creato il Deinit().
...
#warning acemor: attenzione al >> 8 che manca
uint32_t encoderGetUncalibrated(void)
{
...
uint32_t encoderGetElectricalOffset()
{
