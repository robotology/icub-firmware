...
static void s_eo_inertials3_build_maps(EOtheInertials3* p, uint64_t enablemask);
...
static eObool_t s_eo_inertials3_get_id(eObrd_canlocation_t loc, eOas_inertial_type_t type, uint16_t *id);
...
#if 0
    commented out because the service config of inertials3 is different
...
//    }
    ...
    // we assume that 
    s_eo_inertials3_build_maps(p, 0xffffffffffffffff);
...
extern eOresult_t eo_inertials3_Config(EOtheInertials3 *p, eOas_inertial_config_t* config)
{
...
extern eOresult_t eo_inertials3_AcceptCANframe(EOtheInertials3 *p, eOas_inertial_type_t type, eOcanframe_t *frame, eOcanport_t port)
{
    if(NULL == p)
    {
        return(eores_NOK_nullpointer);
...
//    {
//        // send up diagnostics
//        // error -> "Object EOtheInertials3 has received a message from an unsupported inertial"
//        return(eores_OK);        
//    }
//    
