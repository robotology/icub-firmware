...
// --------------------------------------------------------------------------------------------------------------------
#include "EOappEncodersReader_mc4.h"
...
static void s_eo_appEncReader_configureQuadEnc(uint8_t quad_enc_mask);
...
extern EOappEncReader* eo_appEncReader_New(eOappEncReader_cfg_t *cfg)
{
    ...
    //Init of quadrature encoders
	s_eo_appEncReader_configureQuadEnc(cfg->quad_enc_mask);
...
{
    //Keep only the 4 LSB
    uint8_t mask = quad_enc_mask & 0x0F;
    
    for (uint8_t i = 0; i < 4; i++)
    {
        if((mask >> i) & 0x01)
            hal_quad_enc_single_init(i);
    }
}
/*
static void s_eo_appEncReader_check(EOappEncReader *p)
