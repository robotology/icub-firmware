...
namespace RfeApp {
    ...
    constexpr uint8_t expressionMaxNum = embot::core::tointegral(Expression_t::last) + 1; 
    ...
    constexpr uint8_t colormaxNum = embot::core::tointegral(Color::last) + 1;
        
    // use consecutive values 
    enum class Brightness : uint8_t { dark = 0, minimum, low, medium, high, maximum, last = maximum, none = 255 };
    constexpr uint8_t brightnessMaxNum = embot::core::tointegral(Brightness::last) + 1; 
        
    constexpr Expression_t toexpression(const uint32_t e)
    {
        return (e>embot::core::tointegral(Expression_t::last)) ? Expression_t::none : static_cast<Expression_t>(e);        
    }
                            
    constexpr Color tocolor(const uint32_t c)
    {
        return (c>embot::core::tointegral(Color::last)) ? Color::none : static_cast<Color>(c);
    }
    ...
    constexpr uint32_t tovalueofcolor(Color c)
    {
        ...
        return (c == Color::none) ? 0x000000 : _map2value[embot::core::tointegral(c)];        
    }
    

    constexpr Brightness tobrightness(const uint32_t b)
    {
        return (b>embot::core::tointegral(Brightness::last)) ? Brightness::none : static_cast<Brightness>(b);
    }
    
    constexpr uint8_t tovalueofbrightness(Brightness b)
    {
        constexpr uint8_t _map2v[] = { 0, 4, 16, 64, 96, 127 };
        return (b == Brightness::none) ? 0 : _map2v[embot::core::tointegral(b)];        
    }    
