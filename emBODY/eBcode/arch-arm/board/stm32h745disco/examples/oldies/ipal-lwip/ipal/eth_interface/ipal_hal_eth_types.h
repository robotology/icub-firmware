...
typedef struct 
{
	...
	/** 
	 * This structure holds the callback on "transmission done"
	 */
	typedef struct 
	{
		/* Frame TX done CB */
		void (*frame_tx_done_cb)(void);
		
	} ipal_eth_onframe_transmission_t;
...
#if (0)
	/**
	 * COMPATIBLE (castable) to hal_hal_eth_stackIPhookfunctions_t	
	 * This structure is compatible with hal_eth_stackIPhookfunctions_t
	 */
	typedef struct 
	{
		/* Move to higher layer */
		...
		/* Move to lower layer */
		uint32_t (*moveframe2lowerlayer)(ipal_eth_frame_t *genframe, uint8_t *outputbuffer);
...
typedef struct 
{
  ...
  uint8_t                       	dummy_1;   
	...
	//ipal_eth_onframe_transmission_t	*onframetx; 
	uint64_t												macaddress;
} ipal_eth_cfg_t;
