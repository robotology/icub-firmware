...
#warning acemor added:

#if defined(embotapp_UDPDEMOBASIC)
#include "main-embot.h"
#elif defined(embotapp_UDPDEMOIPNET)
...
#warning acemor removed
/* The thread who calls ipal_sys_process */
//extern osThreadId_t net_thread_id;

#warning acemor removed
/* Net thread flags id */
//extern osEventFlagsId_t net_thread_flags_id;
...
static ipal_eth_cfg_t 		ipal_eth_cfg;
 ...
 */
static void ipal_to_arm_mac_addr(ipal_eth_cfg_t *ipal_eth_cfg, ARM_ETH_MAC_ADDR *arm_eth_mac_addr )
{
...
{ 
	...
	memcpy(&ipal_eth_cfg, cfg, sizeof(ipal_eth_cfg_t));
  ...
  if (PHYDRV->Initialize (MACDRV->PHY_Read, MACDRV->PHY_Write) == ARM_DRIVER_OK) {
    ...
    #warning marco.accame: TODO: solve the case when no link is available (RJ45 not plugged)
...
ipal_result_t ipal_hal_eth_stm32h7_sendframe(void *frame)
{
	...
	ipal_eth_frame_t *eth_frame = (ipal_eth_frame_t *)frame; 
	...
	MACDRV->SendFrame(eth_frame->payload, eth_frame->length, flags);
...
///* Wake up the thread who handle the incoming frame */
//void ipal_signal_rx_frame(void)
//{
//    #warning acemor FILL IT
////	osEventFlagsSet (net_thread_flags_id, NET_THREAD_RX_EVENT);	
//}
...
//{
//    #warning acemor FILL IT
////	osEventFlagsSet (net_thread_flags_id, NET_THREAD_TX_EVENT);	
//}

#warning acemor FILL IT

/* Signal RX event (unblocks the waiting task, who will run ipal_sys_process_communication */
void ipal_signal_rx_frame(void)
{
    // marco.accame: for the udpdemo-basic i must send an event to the thread. 
    // to do it i use an external function called:
    eom_lwip_set_rx_event();
}

#warning acemor FILL IT
	
/* Signal TX event (unblocks the waiting task, who will run ipal_sys_process_communication */
void ipal_signal_tx_frame(void)
{
    // marco.accame: for the udpdemo-basic i must send an event to the thread. 
    // to do it i use an external function called:
    eom_lwip_set_tx_event();
}
