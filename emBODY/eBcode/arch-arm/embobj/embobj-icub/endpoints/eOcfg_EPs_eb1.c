...
{  
    ...
    {   // mc-leftarm-upper
        EO_INIT(.endpoint)                          EOK_cfg_nvsEP_mc_leftupperarm_EP,
...
static uint16_t s_hash(uint16_t ep)
{
    ...
    else if(EOK_cfg_nvsEP_mc_leftupperarm_EP == r)
    {
        return(1);
    }
...
static uint16_t s_eo_cfg_nvsEP_eb1_hashfunction_ep2index(uint16_t ep)
{
    ...
    { 
        EOK_cfg_nvsEP_mngmnt_EP,        EOK_cfg_nvsEP_mc_leftupperarm_EP,       EOK_cfg_nvsEP_as_wholebody_EP
    };
