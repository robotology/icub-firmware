...
void MController_config_board(const eOmn_serv_configuration_t* brd_cfg)
{
    ...
    for (int k=0; k<o->nJoints; ++k)
    {
        ...
        // each motor has an actuation type which depends on actuation_type
    ...
    // marco.accame on 08 jan 2021: it is ok to use MAX_JOINTS_PER_BOARD and MAX_ENCODS_PER_BOARD because the matrices are allocated upon their values
    for (int i=0; i<MAX_JOINTS_PER_BOARD; ++i)
    {
        for (int k=0; k<MAX_ENCODS_PER_BOARD; ++k)
        {
