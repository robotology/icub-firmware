...
#define __EMBOT_APP_BLDC_MBD_INTERFACE_H_
    ...
    this file contains interface vs the Model Based Design code generated by matlab
    so that the C++ application can call invariant API, for instance to init MBD, 
    tick it at 1 ms and call its foc.
    
    we shall make an effort to keep this interface fixed even if the MBD code API may change.
    
    it is file embot_app_bldc_MBD_api.h which keeps the MBD API.
    
    in the same file we also define a:
    ```C++
    namespace embot::app::bldc::mbd::interface { constexpr size_t maxMOTORs {2};  }
    ```
#endif
...
static_assert((embot::app::bldc::mbd::interface::maxMOTORs == 2), "");
