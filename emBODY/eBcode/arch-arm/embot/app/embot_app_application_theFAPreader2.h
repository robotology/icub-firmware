...
namespace embot { namespace app { namespace application {
    ...
    class theFAPreader2 : public CANagentPOS
    {
    public:
        static theFAPreader2& getInstance();
    
        // the measurement units is:
        // centideg = 0.01 degrees for sensorType::tlv
        using Position = int32_t; 
    
        static constexpr Position valueOfPositionACQUISITIONnotvalid {1000*100}; // 1000 degrees or 
        
        static constexpr std::uint8_t numberofpositions = 4; 
        ...
        // a timer initted by theFAPreader2 emits the event transmit in regular mode as commanded by Message_POS_TRANSMIT
        ...
        // the way askdata[s] is emitted by theFAPreader2 ... depends on the parallelism of the chips
        // if all are independent, theFAPreader2 may emit all the askdata[] events at the same time.
        // if they all are on the same bus, it will emit one after another. 
        // if as in the pmc board they share a bus two by two, then theFAPreader2 can start acquitions in two times
