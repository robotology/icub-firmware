...
//        I2C_HandleTypeDef* handles[embot::core::tointegral(I2CE::maxnumberof)];           
        PrivateData() { }
    };
    ...
    static result_t s_wait_for_transaction_completion(I2CE b, embot::core::relTime timeout);   
    static result_t s_DMA_write_transaction_start(I2CE b, ADR adr, Reg reg, const embot::core::Data &content, const embot::core::Callback &oncompletion = embot::core::Callback(nullptr, nullptr));
    static result_t s_DMA_read_transaction_start(I2CE b, ADR adr, Reg reg, embot::core::Data &destination, const embot::core::Callback &oncompletion = embot::core::Callback(nullptr, nullptr));        
    ...
    result_t init(I2CE b, const Config &config)
    {
        ...
        //s_privatedata.handles[index] = embot::hw::i2ce::bsp::getBSP().getPROP(b)->handle;
    ...
    result_t tx(embot::hw::I2CE b, ADR adr, const embot::core::Data &content, embot::core::relTime timeout)
    {
        ...
        std::uint8_t index = embot::core::tointegral(b);             
        embot::hw::i2ce::s_privatedata.transaction[index].ongoing = true;    
        volatile HAL_StatusTypeDef r {HAL_OK};   
#warning TODO: change w/ gpio-emulated implementation -> HAL_I2C_Master_Transmit() [blocking w/ timeout]    
        // r = HAL_I2C_Master_Transmit(s_privatedata.handles[index], static_cast<uint16_t>(adr), reinterpret_cast<uint8_t*>(content.pointer), static_cast<uint16_t>(content.capacity), timeout/1000);                       
...
#warning TODO: change w/ gpio-emulated implementation -> HAL_I2C_IsDeviceReady() [blocking w/ timeout] 
        HAL_StatusTypeDef status {HAL_OK};        
        // status = HAL_I2C_IsDeviceReady(s_privatedata.handles[index], adr, 1, remaining/1000);            
    ...
    result_t read(I2CE b, ADR adr, Reg reg, embot::core::Data &destination, const embot::core::Callback &oncompletion)
    {
        ...
        result_t r = s_DMA_read_transaction_start(b, adr, reg, destination, oncompletion);
        return r;
    }    
    ...
    result_t write(I2CE b, ADR adr, Reg reg, const embot::core::Data &content, const embot::core::Callback &oncompletion)
    {
        ...
        result_t r = s_DMA_write_transaction_start(b, adr, reg, content, oncompletion);
        return r;
    }    
    ...
    static result_t s_DMA_write_transaction_start(I2CE b, ADR adr, Reg reg, const embot::core::Data &content, const embot::core::Callback &oncompletion)
    { 
        result_t res = resNOK; 
        ...
        #warning TODO: change w/ gpio-emulated implementation -> HAL_I2C_Master_Transmit_DMA() [non blocking]
        HAL_StatusTypeDef r {HAL_OK};
//        r = HAL_I2C_Master_Transmit_DMA(s_privatedata.handles[index], adr, static_cast<std::uint8_t*>(s_privatedata.transaction[index].txbuffer), s_privatedata.transaction[index].txbuffersize);        
    ...
    static result_t s_DMA_read_transaction_start(I2CE b, ADR adr, Reg reg, embot::core::Data &destination, const embot::core::Callback &oncompletion)
    {
        result_t res = resNOK;
        ...
        if(reg.address == Reg::addressNONE)
        {
            ...
            HAL_StatusTypeDef r {HAL_OK};
#warning TODO: change w/ gpio-emulated implementation -> HAL_I2C_Master_Receive_DMA() [non blocking]   
//            I2C_HandleTypeDef* hi2cx = embot::hw::i2ce::s_privatedata.handles[index];
//            r = HAL_I2C_Master_Receive_DMA( hi2cx, 
//                                        embot::hw::i2ce::s_privatedata.transaction[index].addr, 
//                                        static_cast<std::uint8_t*>(embot::hw::i2ce::s_privatedata.transaction[index].recdata.pointer),  
//                                        static_cast<std::uint16_t>(embot::hw::i2ce::s_privatedata.transaction[index].recdata.capacity)
//                                    );  
            res = (HAL_OK == r) ? resOK : resNOK;                                        
        }
        else
    ...
    {
        embot::hw::i2ce::s_privatedata.transaction[index].ongoing = false;        
        embot::hw::i2ce::s_privatedata.transaction[index].oncompletion.execute();                                
    }    
...
//{        
//    embot::hw::I2CE id = embot::hw::i2ce::getBSP().toID({I2cHandle, embot::hw::i2ce::Speed::none});
//    
...
//    
//               
//    if(false == embot::hw::i2ce::s_privatedata.transaction[index].recdata.isvalid())
//    {
//        // it is a write operation: the transaction is terminated
//        embot::hw::i2ce::ontransactionterminated(index);        
//    }
//    else
//    {
...
// we could move them into the embot_hw_bsp.cpp file (but oustide the hw::bsp::i2ce namespace !!!!)
