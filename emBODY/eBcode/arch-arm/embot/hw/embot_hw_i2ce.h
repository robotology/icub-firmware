...
//    enum class Signal { SCK = 0, SDA = 1, NumberOf = 2 };
    ...
    struct Config
    {   // so far we cannot initialize I2CE with a given speed at runtime   
    ...
    struct Descriptor
    {
        embot::hw::I2CE bus {embot::hw::I2CE::none};
        ADR adr {0};
        constexpr Descriptor() = default;
        constexpr Descriptor(embot::hw::I2CE b, ADR a = 0) : bus(b), adr(a) {}
        constexpr bool isvalid() const { return (embot::hw::I2CE::none != bus) && (0 != adr); }              
    };
    ...
    bool isbusy(embot::hw::I2CE b, embot::core::relTime timeout, embot::core::relTime &remaining);
    ...
    bool ping(embot::hw::I2CE b, ADR adr, embot::core::relTime timeout);
    ...
    result_t read(embot::hw::I2CE b, ADR adr, Reg reg, embot::core::Data &destination, embot::core::relTime timeout);
    ...
    // before any activity this functions waits until isbusy(b, timeout, remaining) returns false.
    result_t write(embot::hw::I2CE b, ADR adr, Reg reg, const embot::core::Data &content, embot::core::relTime timeout);
    ...
    // before any activity this functions waits until isbusy(b, timeout, remaining) returns false.
    result_t receive(embot::hw::I2CE b, ADR adr, embot::core::Data &destination, embot::core::relTime timeout);    
    ...
    result_t read(embot::hw::I2CE b, ADR adr, Reg reg, embot::core::Data &destination, const embot::core::Callback &oncompletion);
    ...
    // this funtions returns resOK if the hardware can succesfully start its operations and resNOK in any failure case.
    result_t write(embot::hw::I2CE b, ADR adr, Reg reg, const embot::core::Data &content, const embot::core::Callback &oncompletion = embot::core::Callback(nullptr, nullptr));
    ...
    // the specified callback which can be used to alert a thread. 
    // this funtions returns resOK if the hardware can succesfully start its operations and resNOK in any failure case.
    result_t receive(embot::hw::I2CE b, ADR adr, embot::core::Data &destination, const embot::core::Callback &oncompletion);  
