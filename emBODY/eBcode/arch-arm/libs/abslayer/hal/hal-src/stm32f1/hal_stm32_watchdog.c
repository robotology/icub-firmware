...
extern uint32_t hal_watchdog_hid_getsize(const hal_cfg_t *cfg)
{
    // no memory needed
    return(0);
}

extern hal_result_t hal_watchdog_hid_setmem(const hal_cfg_t *cfg, uint32_t *memory)
{
    // no memory needed
//    if(NULL == memory)
//    {
...
static hal_boolval_t s_hal_watchdog_supported_is(hal_watchdog_t watchdog)
{
    return(hal_base_hid_byte_bitcheck(hal_brdcfg_watchdog__supported_mask, HAL_watchdog_t2index(watchdog)) );
}
