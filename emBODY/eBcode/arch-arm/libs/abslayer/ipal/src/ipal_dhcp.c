...
extern ipal_result_t ipal_dhcp_restart(void)
{
    ...
    return(ipal_res_NOK_generic); // dhcp_init() is not available in rtl
    
    // if i am in here, it means that we have teh cfg, but also that the dhcp is enabled and initted,
    // thus we have the proper functions to run
    ...
    dhcp_disable();
    //dhcp_init(); not available
    //#warning --> in rtl is missing dhcp_init()
