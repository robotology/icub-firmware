...
bool_t osRtxThreadStartup (void) {
  ...
  osRtxInfo.thread.idle = osRtxThreadId(
    ...
    // we force a timer thread
    if((NULL == osRtxConfig.timer_setup) || (NULL == osRtxConfig.timer_thread))
    {
        osRtxConfig.timer_setup = osRtxTimerSetup;
        osRtxConfig.timer_thread = osRtxTimerThread;        
    }
#endif    
