...
extern hal_result_t hal_mux_init(hal_mux_t id, const hal_mux_cfg_t *cfg)
{
    ...
    memcpy(&intitem->enable, &hal_brdcfg_mux__theconfig.gpiomap[HAL_mux_id2index(id)].gpio_enable.gpio, sizeof(hal_gpio_t));
    memcpy(&intitem->sel0,   &hal_brdcfg_mux__theconfig.gpiomap[HAL_mux_id2index(id)].gpio_sel0.gpio, sizeof(hal_gpio_t));
    memcpy(&intitem->sel1,   &hal_brdcfg_mux__theconfig.gpiomap[HAL_mux_id2index(id)].gpio_sel1.gpio, sizeof(hal_gpio_t));
...
static hal_boolval_t s_hal_mux_supported_is(hal_mux_t id)
{
    return((hal_boolval_t)hl_bits_byte_bitcheck(hal_brdcfg_mux__theconfig.supported_mask, HAL_mux_id2index(id)));
}


static void s_hal_mux_initted_set(hal_mux_t id)
{
    hl_bits_byte_bitset(&s_hal_mux_theinternals.initted, HAL_mux_id2index(id));
}


static hal_boolval_t s_hal_mux_initted_is(hal_mux_t id)
{
    return((hal_boolval_t)hl_bits_byte_bitcheck(s_hal_mux_theinternals.initted, HAL_mux_id2index(id)));
}
