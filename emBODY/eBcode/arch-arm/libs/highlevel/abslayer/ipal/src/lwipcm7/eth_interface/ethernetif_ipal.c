 ...
 */
static ipal_hal_eth_onframereception_t ipal_eth_onframe_reception = 
{
	/* Frame new - Unused */
	.frame_new = NULL,
...
void ethernetif_IPAL_low_level_init(void)
{
	...
	ipal_hal_eth_cfg_t ipal_eth_cfg = 
	{
		.priority 		= 0,
		.dummy          = {0, 0, 0},      
		.onframerx		= &ipal_eth_onframe_reception,
		//.onframetx		= &ipal_eth_onframe_transmission,
		.macaddress		= ipal_base_hid_cfg2_eth.eth_mac
	};
	
	/* Get a reference to IPAL external functions */
	//ipal_cfg2_extfn_t *cfgfn2 = &ipal_base_hid_cfg2_extfn2;
    ipal_cfg2_eth_t *cfg2eth = &ipal_base_hid_cfg2_eth;

	/* Hardware dependent initialization function */
	res = cfg2eth->hal_eth_init(&ipal_eth_cfg);
	
	if(ipal_res_OK != res)
	{
		ipal_base_hid_on_fatalerror(ipal_error_generic, "low_level_init: cfgfn2.hal_eth_init() failed ");
	}	
}
...
ipal_result_t ethernetif_IPAL_low_level_output(ipal_hal_eth_frame_t *frame)
{
    ...
    res = cfg2eth->hal_eth_sendframe(frame);
...
{	
    ...
    #warning TODO: remove ipal_hal_eth_aux_functions
    // this function does the same as legacy usr_on_ethframe_received()
    // why is it used instead? 
		/* Signal external thread that a new frame is available */
//		ipal_hal_eth_aux_functions.signal_rx_frame();
    if(NULL != ipal_base_hid_cfg2_extfn2.signal_rx_frame)
    {
        ipal_base_hid_cfg2_extfn2.signal_rx_frame();
    }
