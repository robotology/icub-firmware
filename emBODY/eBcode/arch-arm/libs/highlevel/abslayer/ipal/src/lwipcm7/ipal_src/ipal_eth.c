 ...
 */
uint32_t ipal_eth_hid_getsize(const ipal_cfg2_t *cfg2)
{
    // dont need memory	
	return(0);
}
 ...
 * See header file for details
 */
ipal_result_t ipal_eth_hid_setmem(const ipal_cfg2_t *cfg2, uint32_t *memory)
{
 ...
 * See header file for details
 */
ipal_result_t ipal_eth_hid_init(ipal_cfg2_t *cfg2)
{
    if(NULL == cfg2->eth)
    {
		ipal_base_hid_on_fatalerror(ipal_error_missingconfiguration, "ipal_eth_hid_init(): missing eth config");
		return(ipal_res_NOK_generic);        
    }
    
	if((NULL == cfg2->eth->hal_eth_init) || (NULL == cfg2->eth->hal_eth_enable) ||
       (NULL == cfg2->eth->hal_eth_disable) || (NULL == cfg2->eth->hal_eth_sendframe) ||
       (NULL == cfg2->eth->get_frame) || (NULL == cfg2->eth->get_frame_size) 
      )
	{
 ...
 * See header file for details
 */
ipal_result_t ipal_eth_hid_vars_init(const ipal_cfg2_t *cfg2)
{
...
ipal_result_t ipal_eth_hid_run(void)
{
	...
	ipaddr.addr = ipal2lwip_ipv4addr(ipal_base_hid_cfg2_eth.eth_ip);
	mask.addr = ipal2lwip_ipv4addr(ipal_base_hid_cfg2_eth.eth_mask);
	gw_addr.addr = 0;
	...
	memcpy(&s_ipal_eth_netif.netif.hwaddr, &ipal_base_hid_cfg2_eth.eth_mac, 6);
