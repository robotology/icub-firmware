...
uint8_t REG_Value = 0;
#define SAMPLES 10
uint8_t REG_max_count = SAMPLES-1;     
...
void PGA308_init(void){
	...
	HAL_GPIO_WritePin(ENVDDA_GPIO_Port, ENVDDA_Pin, GPIO_PIN_SET);
...
void PGA308_DefaultConfig(void){
  ...
  for(channel=1; channel<=STRAIN_CHANNELS; channel++){
