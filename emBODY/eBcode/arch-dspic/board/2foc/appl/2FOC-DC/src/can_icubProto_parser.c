...
extern volatile int calibration_wdog;
extern volatile long calibration_pos_max;
extern volatile long calibration_pos_min;
...
static int s_canIcubProtoParser_parse_pollingMsg(tCanData *rxpayload, unsigned char rxlen, tCanData *txpayload, unsigned char *txlen)
{
    ...
    *txlen=0;
    ...
    if (cmd == ICUBCANPROTO_POL_MC_CMD__SET_MOTOR_CONFIG)
    {
        ...
        if (MotorConfig.has_qe) QEconfig(rxpayload->w[1]);
    ...
    */
    ...
    if (cmd == ICUBCANPROTO_POL_MC_CMD__CALIBRATE_ENCODER)
    {
        calibration_wdog = 5000;
        calibration_pos_max = -32000;
        calibration_pos_min =  32000;
