...
// Timer 2 isr, used to do velocity calculation.
{
  ...
  if(VelocityParm.Initializing){
    ...
    {
      VelocityParm.positions_buffer[i] = Current_position;
    }

     VelocityParm.Initializing = 0;
  }
 ...
 // finish calculate mean by dividing by number of elements accumulated
  ...
  // previous value; 
  VelocityParm.Velocity = (vel + VelocityParm.Velocity)/2;
...
// Initializes variables for velocity calculation module
  ...
  // Defer hi-load initialization to interruptible context because
  // this function is called in behalf of 2FOC deep loop
  // through the rotor alignment layer.
  VelocityParm.Initializing = 1;
