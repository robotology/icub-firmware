...
typedef struct      // 8 bytes
{
    ...
    uint16_t        dummy1;
    uint32_t        checkdata;
} eEcheck_t;        EECOMMON_VERIFYsizeof(eEcheck_t, 8);
...
static eEresult_t s_ee_common_permanent_data_erase(const eEstorage_t *strg, hal_eeprom_t haleeprom2use, uint32_t size);
...
extern eEresult_t ee_permdata_erase(const eEpermdataInfo_t *pdatainfo)
{    
    if(NULL == pdatainfo)
    {
        return(ee_res_NOK_generic);
    ...
    s_ee_common_permanent_data_erase(pdatainfo->strg, pdatainfo->haleeprom2use, pdatainfo->strg->size);
...
extern eEresult_t ee_permdata_synch(const eEpermdataInfo_t *pdatainfo, const void *data, uint32_t size)
{
    ...
    // if data from storage is not coherent with its check, then ... re-synchronise
...
static eEresult_t s_ee_common_permanent_data_erase(const eEstorage_t *strg, hal_eeprom_t haleeprom2use, uint32_t size)
{
    ...
    res = hal_eeprom_erase(haleeprom2use, strg->addr, size);
