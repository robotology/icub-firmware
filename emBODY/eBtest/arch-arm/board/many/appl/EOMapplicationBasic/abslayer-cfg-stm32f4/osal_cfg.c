...
extern void* osal_ext_calloc(uint32_t s, uint32_t n)
{
    ...
    snprintf(str, sizeof(str), "osal_ext_calloc %d bytes (%d on heap): [%x, %x]", n*s, eo_common_msize(ret), (uint32_t)ret, (uint32_t)ret+eo_common_msize(ret));
    hal_trace_puts(str);
...
extern void* osal_ext_realloc(void* m, uint32_t s)
{
    ...
    size_t old = eo_common_msize(m);
    void* ret = realloc(m, s);
        
    snprintf(str, sizeof(str), "osal_ext_realloc %d bytes (%d -> %d): [%x, %x]", s, old, eo_common_msize(ret), (uint32_t)ret, (uint32_t)ret+eo_common_msize(ret));
    hal_trace_puts(str);
...
{
    char str[64];
    snprintf(str, sizeof(str), "osal_ext_free (%d on heap): [%x, %x]", eo_common_msize(m), (uint32_t)m, (uint32_t)m+eo_common_msize(m));
    hal_trace_puts(str);
