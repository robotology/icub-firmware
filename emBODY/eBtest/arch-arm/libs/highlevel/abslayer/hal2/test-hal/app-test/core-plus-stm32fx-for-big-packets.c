...
static uint8_t udpmsgback[2048] = {0};
...
static void s_udp_onreception(void *arg, ipal_udpsocket_t *skt, ipal_packet_t *pkt, ipal_ipv4addr_t adr, ipal_port_t por)
{
    ...
    uint16_t maxsize = (pkt->size > sizeof(udpmsgback)) ? (sizeof(udpmsgback)) : (pkt->size);
    
    memcpy(udpmsgback, pkt->data, maxsize);
...
#if     defined(EXECUTE_TEST_ETH_UDP_RECEIVEANDREPLY)    
static void test_periph_eth_udp_reply(uint16_t txsize)
{
    char msg[128] =  {0};
    ...
    {
        num++;
        snprintf(msg, sizeof(msg)-1, "eth: transmitted a packet with data = {0x%x, 0x%x, 0x%x, 0x%x} and size %d to remote host %d.%d.%d.%d:%d", 
...
{   
    ...
    const uint16_t replyudpsize = 1600; // max is 1460
    ...
    for(;;)
    {
        ...
        {
            s_reply_udp_msg = 0;
            test_periph_eth_udp_reply(replyudpsize);
