...
{
	_val=get_absolute_real_position_abs_ssi(jnt);
	max_real_position[jnt]=_val;
}
...
{	
	...
	if ((_val>= max_real_position[jnt] ) && (_val <= 4095))
	{
		_val=_val- max_real_position[jnt]; 
	}
	else
	{
		_val=_val+ (4095-max_real_position[jnt])+1;
	}
