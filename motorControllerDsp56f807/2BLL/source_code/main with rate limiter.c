...
//#define RATE_LIMITER
#ifdef  RATE_LIMITER
			#warning "rate limiter on"			
			//rate limiter compensation?
			if (PWMoutput[i]>PWMoutput_old[i]+10) PWMoutput[i] = PWMoutput_old[i]+10;
			if (PWMoutput[i]<PWMoutput_old[i]-10) PWMoutput[i] = PWMoutput_old[i]-10;
#endif
*/
...
{
    #define CHOOSE_HARD_BOUND 100

	static UInt32 TrqLimitCount =0;
 	char   signKp = (_kp[i]>0?1:-1);
 	char   signDc = (DutyCycle[i].Dir==0?1:-1);
	  	...
	  	_control_mode[i] == MODE_IMPEDANCE_VEL)
 		{
	 		if  (_position[i] > _max_position[i])
	 		{
	 			...
	 			if (signKp*signPWM >0 )
				{
					...
					PWM[i] = PWMc;
					//PWM[i] = CHOOSE_HARD_BOUND * -signKp;
					//if (_kp[i]>0) PWM[i] = -CHOOSE_HARD_BOUND;
					//else 		  PWM[i] =  CHOOSE_HARD_BOUND;	
				}
				TrqLimitCount++;	 
				if (TrqLimitCount>=200)
				{
					//#ifdef DEBUG_CONTROL_MODE
						...
						can_printf("kp:%d Dc:%d B:%d A:%f", signKp, signDc, PWMb, PWM[i]);	
						TrqLimitCount=0;
				    //#endif 
				}			
	 		}
