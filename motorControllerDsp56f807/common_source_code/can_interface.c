...
#define CAN_TEMP16 ikk
...
#if VERSION==0x0157 || VERSION==0x0150 || VERSION == 0x0351 || VERSION==0x0147 || VERSION==0x0140 || VERSION==0x0250 || VERSION==0x0257
    ...
    VERSION == 0x0219 || VERSION == 0x0250 || VERSION == 0x0251 || VERSION == 0x0252 || VERSION == 0x0254 || VERSION == 0x0257 
			if (_canmsg.CAN_ID_class == ICUBCANPROTO_CLASS_PERIODIC_ANALOGSENSOR)
			{
				switch(_canmsg.CAN_ID_dst)
				{
...
#elif (VERSION == 0x0128 || VERSION == 0x0130 || VERSION == 0x0228 || VERSION == 0x0230)
			...
			if (_canmsg.CAN_ID_class == ICUBCANPROTO_CLASS_BOOTLOADER)
			{
			...
			else if (_canmsg.CAN_ID_class == ICUBCANPROTO_CLASS_POLLING_MOTORCONTROL)
			{
				int axis = get_axis();
				...
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__POSITION_MOVE, CAN_POSITION_MOVE_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__VELOCITY_MOVE, CAN_VELOCITY_MOVE_HANDLER)
				
				//Low priority (occasional) messages.
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__NO_MESSAGE, CAN_NO_MESSAGE_HANDLER)
			    //HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__CONTROLLER_RUN, CAN_CONTROLLER_RUN_HANDLER)   //deprecated
				//HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__CONTROLLER_IDLE, CAN_CONTROLLER_IDLE_HANDLER) //deprecated
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__TOGGLE_VERBOSE, CAN_TOGGLE_VERBOSE_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__CALIBRATE_ENCODER, CAN_CALIBRATE_ENCODER_HANDLER)
				//HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__ENABLE_PWM_PAD, CAN_ENABLE_PWM_PAD_HANDLER)   //deprecated
				//HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__DISABLE_PWM_PAD, CAN_DISABLE_PWM_PAD_HANDLER) //deprecated
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_CONTROL_MODE, CAN_GET_CONTROL_MODE_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_CONTROL_MODE, CAN_SET_CONTROL_MODE_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__MOTION_DONE, CAN_MOTION_DONE_HANDLER)
			
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__WRITE_FLASH_MEM, CAN_WRITE_FLASH_MEM_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__READ_FLASH_MEM, CAN_READ_FLASH_MEM_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_ADDITIONAL_INFO, CAN_GET_ADDITIONAL_INFO_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_ADDITIONAL_INFO, CAN_SET_ADDITIONAL_INFO_HANDLER)
			
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_ENCODER_POSITION, CAN_GET_ENCODER_POSITION_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_DESIRED_POSITION, CAN_SET_DESIRED_POSITION_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_DESIRED_POSITION, CAN_GET_DESIRED_POSITION_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_DESIRED_VELOCITY, CAN_SET_DESIRED_VELOCITY_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_DESIRED_VELOCITY, CAN_GET_DESIRED_VELOCITY_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_DESIRED_ACCELER, CAN_SET_DESIRED_ACCELER_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_DESIRED_ACCELER, CAN_GET_DESIRED_ACCELER_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_DESIRED_TORQUE, CAN_SET_DESIRED_TORQUE_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_DESIRED_TORQUE, CAN_GET_DESIRED_TORQUE_HANDLER)
				
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_DEBUG_PARAM, CAN_SET_DEBUG_PARAM_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_DEBUG_PARAM, CAN_GET_DEBUG_PARAM_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_SPEED_ESTIM_SHIFT, CAN_SET_SPEED_ESTIM_SHIFT_HANDLER) 

				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_ENCODER_POSITION, CAN_SET_ENCODER_POSITION_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_ENCODER_VELOCITY, CAN_GET_ENCODER_VELOCITY_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_COMMAND_POSITION, CAN_SET_COMMAND_POSITION_HANDLER)
				
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_P_GAIN, CAN_SET_P_GAIN_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_P_GAIN, CAN_GET_P_GAIN_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_D_GAIN, CAN_SET_D_GAIN_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_D_GAIN, CAN_GET_D_GAIN_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_I_GAIN, CAN_SET_I_GAIN_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_I_GAIN, CAN_GET_I_GAIN_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_ILIM_GAIN, CAN_SET_ILIM_GAIN_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_ILIM_GAIN, CAN_GET_ILIM_GAIN_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_OFFSET, CAN_SET_OFFSET_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_OFFSET, CAN_GET_OFFSET_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_SCALE, CAN_SET_SCALE_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_SCALE, CAN_GET_SCALE_HANDLER)
			
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_POSITION_MOVE_REF, CAN_GET_POSITION_MOVE_REF_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_VELOCITY_MOVE_REF, CAN_GET_VELOCITY_MOVE_REF_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_PID_OUTPUT, CAN_GET_PID_OUTPUT_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_PID_ERROR, CAN_GET_PID_ERROR_HANDLER)
				
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_MIN_POSITION, CAN_SET_MIN_POSITION_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_MIN_POSITION, CAN_GET_MIN_POSITION_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_MAX_POSITION, CAN_SET_MAX_POSITION_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_MAX_POSITION, CAN_GET_MAX_POSITION_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_OFFSET_ABS_ENCODER, CAN_SET_OFFSET_ABS_ENCODER_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_OFFSET_ABS_ENCODER, CAN_GET_OFFSET_ABS_ENCODER_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_MAX_VELOCITY, CAN_SET_MAX_VELOCITY_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_MAX_VELOCITY, CAN_GET_MAX_VELOCITY_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_CALIBRATION_STATUS, CAN_GET_CALIBRATION_STATUS_HANDLER)
			
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_TLIM, CAN_SET_TLIM_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_TLIM, CAN_GET_TLIM_HANDLER)
			
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_BOARD_ID, CAN_SET_BOARD_ID_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_BOARD_ID, CAN_GET_BOARD_ID_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_TORQUE_SOURCE, CAN_SET_TORQUE_SOURCE_HANDLER)
				
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_CURRENT_LIMIT, CAN_SET_CURRENT_LIMIT_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_BCAST_POLICY, CAN_SET_BCAST_POLICY_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_ERROR_STATUS, CAN_GET_ERROR_STATUS_HANDLER)
				...
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_PWM_LIMIT, CAN_SET_PWM_LIMIT_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_PWM_LIMIT, CAN_GET_PWM_LIMIT_HANDLER)
				...
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_TORQUE_PIDLIMITS, CAN_GET_TORQUE_PIDLIMITS_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_POS_PID, CAN_SET_POS_PID_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_POS_PID, CAN_GET_POS_PID_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_POS_PIDLIMITS, CAN_SET_POS_PIDLIMITS_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_POS_PIDLIMITS, CAN_GET_POS_PIDLIMITS_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_IMPEDANCE_PARAMS, CAN_SET_IMPEDANCE_PARAMS_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_IMPEDANCE_PARAMS, CAN_GET_IMPEDANCE_PARAMS_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_IMPEDANCE_OFFSET, CAN_SET_IMPEDANCE_OFFSET_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_IMPEDANCE_OFFSET, CAN_GET_IMPEDANCE_OFFSET_HANDLER)
				
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_OPTICAL_ENC_RATIO, CAN_SET_OPTICAL_ENC_RATIO_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_POS_STICTION_PARAMS, CAN_SET_POS_STICTION_PARAMS_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_POS_STICTION_PARAMS, CAN_GET_POS_STICTION_PARAMS_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_TORQUE_STICTION_PARAMS, CAN_SET_TORQUE_STICTION_PARAMS_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_TORQUE_STICTION_PARAMS, CAN_GET_TORQUE_STICTION_PARAMS_HANDLER)
				
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_MOTOR_PARAMS, CAN_SET_MOTOR_PARAMS_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_MOTOR_PARAMS, CAN_GET_MOTOR_PARAMS_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_MAX_MOTOR_POS, CAN_SET_MAX_MOTOR_POS_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_MIN_MOTOR_POS, CAN_SET_MIN_MOTOR_POS_HANDLER)

				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_MODEL_PARAMS, CAN_SET_MODEL_PARAMS_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_MODEL_PARAMS, CAN_GET_MODEL_PARAMS_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_OPENLOOP_PARAMS, CAN_SET_OPENLOOP_PARAMS_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_OPENLOOP_PARAMS, CAN_GET_OPENLOOP_PARAMS_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_INTERACTION_MODE, CAN_SET_INTERACTION_MODE_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_INTERACTION_MODE, CAN_GET_INTERACTION_MODE_HANDLER)
													
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_FIRMWARE_VERSION, CAN_GET_FIRMWARE_VERSION_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__SET_TCFILTER_TYPE, CAN_SET_TCFILTER_TYPE_HANDLER)
				HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_TCFILTER_TYPE, CAN_GET_TCFILTER_TYPE_HANDLER)
														
	//			HANDLE_MSG (ICUBCANPROTO_POL_MC_CMD__GET_ACTIVE_ENCODER_POSITION, CAN_GET_ACTIVE_ENCODER_POSITION_HANDLER)
...
void can_send_broadcast_debug(byte j, int rate)
{
	...
	if (count==rate)
	{
		...
		_canmsg.CAN_messID |= ICUBCANPROTO_PER_MC_MSG__DEBUG;
...
void can_send_broadcast(void)
{
	...
	if ((broadcast_mask[0] & (1<<(ICUBCANPROTO_PER_MC_MSG__POSITION-1))) && _counter == 0)
	{		
		_canmsg.CAN_messID = 0x100;
		_canmsg.CAN_messID |= (_board_ID) << 4;
		_canmsg.CAN_messID |= ICUBCANPROTO_PER_MC_MSG__POSITION;
...
#if (VERSION ==0x0154 || VERSION == 0x0351 || VERSION ==0x0151 || VERSION ==0x0251 || VERSION==0x0254 )
		if(FAULT_ABS0 == 0)
			CAN1_send (_canmsg.CAN_messID, _canmsg.CAN_frameType, _canmsg.CAN_length, _canmsg.CAN_data);
if ((CURRENT_BOARD_TYPE == BOARD_TYPE_BLL) || (CURRENT_BOARD_TYPE == BOARD_TYPE_2BLLDC)) 
			#ifdef USE_ABS_SSI
	...
	if (JN==4 && (broadcast_mask[1] & (1<<(ICUBCANPROTO_PER_MC_MSG__POSITION-1))) && _counter == 0)
	{
	...
	// **************************************************************************/
	if ((broadcast_mask[0] & (1<<(ICUBCANPROTO_PER_MC_MSG__PID_VAL-1)) )  && _counter == 1)
	{
		/* CHANGED: send PID (control) value */
		_canmsg.CAN_messID = 0x100;
		_canmsg.CAN_messID |= (_board_ID) << 4;
		_canmsg.CAN_messID |= ICUBCANPROTO_PER_MC_MSG__PID_VAL;
	...
	if (JN==4 && (broadcast_mask[1] & (1<<(ICUBCANPROTO_PER_MC_MSG__PID_VAL-1))) && _counter == 1)
	{	
			_canmsg.CAN_messID = 0x100;
			_canmsg.CAN_messID |= (_board_ID+1) << 4;
			_canmsg.CAN_messID |= ICUBCANPROTO_PER_MC_MSG__PID_VAL;
	...
	if ((broadcast_mask[0] & (1<<(ICUBCANPROTO_PER_MC_MSG__STATUS-1))) && _counter == 2)
	{
		for (k=0;k<8;k++)
		{
	...
	if ((broadcast_mask[0] & (1<<(ICUBCANPROTO_PER_MC_MSG__CURRENT-1)))  && _counter == 3)
	{
		_canmsg.CAN_messID = 0x100;
		_canmsg.CAN_messID |= (_board_ID) << 4;
		_canmsg.CAN_messID |= ICUBCANPROTO_PER_MC_MSG__CURRENT;
	...
	if (JN== 4 && (broadcast_mask[1] & (1<<(ICUBCANPROTO_PER_MC_MSG__CURRENT-1))) && _counter == 3)
	{
		_canmsg.CAN_messID = 0x100;
		_canmsg.CAN_messID |= (_board_ID+1) << 4;
		_canmsg.CAN_messID |= ICUBCANPROTO_PER_MC_MSG__CURRENT;
	...
	if ((broadcast_mask[0] & (1<<(ICUBCANPROTO_PER_MC_MSG__PID_ERROR-1)))  && _counter == 3) //same counter of ICUBCANPROTO_PER_MC_MSG__CURRENT
	{
		_canmsg.CAN_messID = 0x100;
		_canmsg.CAN_messID |= (_board_ID) << 4;
		_canmsg.CAN_messID |= ICUBCANPROTO_PER_MC_MSG__PID_ERROR;
	...
	if (JN==4 && (broadcast_mask[1] & (1<<(ICUBCANPROTO_PER_MC_MSG__PID_ERROR-1))) && _counter == 3) //same counter of ICUBCANPROTO_PER_MC_MSG__CURRENT
	{
			_canmsg.CAN_messID = 0x100;
			_canmsg.CAN_messID |= (_board_ID+1) << 4;
			_canmsg.CAN_messID |= ICUBCANPROTO_PER_MC_MSG__PID_ERROR;
...
//  @@@RANDAZ: important: CAN_SYNCHRO_STEP has been reduced from 5 to 4 because of CAN_SET_ACTIVE_PID_HANDLER()	
	if ((broadcast_mask[0] & (1<<(ICUBCANPROTO_PER_MC_MSG__VELOCITY-1))) && _counter == 3)
	{
		_canmsg.CAN_messID = 0x100;
		_canmsg.CAN_messID |= (_board_ID) << 4;
		_canmsg.CAN_messID |= ICUBCANPROTO_PER_MC_MSG__VELOCITY;
...
//  @@@RANDAZ: important: CAN_SYNCHRO_STEP has been reduced from 5 to 4 because of CAN_SET_ACTIVE_PID_HANDLER()	
	...
	if ((broadcast_mask[0] & (1<<(ICUBCANPROTO_PER_MC_MSG__MOTOR_POSITION-1))) && _counter == 3)
	{
		_canmsg.CAN_messID = 0x100;
		_canmsg.CAN_messID |= (_board_ID) << 4;
		_canmsg.CAN_messID |= ICUBCANPROTO_PER_MC_MSG__MOTOR_POSITION;
...
//  @@@RANDAZ: important: CAN_SYNCHRO_STEP has been reduced from 5 to 4 because of CAN_SET_ACTIVE_PID_HANDLER()	
	...
	if ((broadcast_mask[0] & (1<<(ICUBCANPROTO_PER_MC_MSG__MOTOR_SPEED-1))) && _counter == 3)
	{
		Int16 casted_motor_speed0 = (Int16)(_motor_speed[0]);
		Int16 casted_motor_speed1 = (Int16)(_motor_speed[1]);
		_canmsg.CAN_messID = 0x100;
		_canmsg.CAN_messID |= (_board_ID) << 4;
		_canmsg.CAN_messID |= ICUBCANPROTO_PER_MC_MSG__MOTOR_SPEED;
...
void set_can_masks()
{
	...
	#if ( VERSION==0x0157 || VERSION==0x0150 || VERSION==0x0147 || VERSION==0x0140 || VERSION==0x0250 || VERSION==0x0257)
		{	
			create_F_M(&filter1, &mask1,ICUBCANPROTO_CLASS_POLLING_MOTORCONTROL,0xFF, _board_ID, ICUBCANPROTO_CLASS_PERIODIC_ANALOGSENSOR, 0xFF, 0xFF);   
			create_F_M(&filter2, &mask2,ICUBCANPROTO_CLASS_BOOTLOADER,  0x00, 0xFF, 	   ICUBCANPROTO_CLASS_PERIODIC_MOTORCONTROL, 	CAN_ID_COUPLED_BOARD, 0xFF);  		
		}
	#elif (VERSION==0x0351)
		if (_board_ID == 1)
		{	
			create_F_M(&filter1, &mask1,ICUBCANPROTO_CLASS_POLLING_MOTORCONTROL,0xFF, _board_ID, ICUBCANPROTO_CLASS_PERIODIC_ANALOGSENSOR, 0xFF, 0xFF);   
			create_F_M(&filter2, &mask2,ICUBCANPROTO_CLASS_BOOTLOADER,  0x00, 0xFF, 	   ICUBCANPROTO_CLASS_PERIODIC_MOTORCONTROL, 	2, 0xFF);  		
		}
		else if (_board_ID == 2)
		{	
			create_F_M(&filter1, &mask1,ICUBCANPROTO_CLASS_POLLING_MOTORCONTROL,0xFF, _board_ID, ICUBCANPROTO_CLASS_PERIODIC_ANALOGSENSOR, 0xFF, 0xFF);   
			create_F_M(&filter2, &mask2,ICUBCANPROTO_CLASS_BOOTLOADER,  0x00, 0xFF, 	   ICUBCANPROTO_CLASS_PERIODIC_MOTORCONTROL, 	1, 0xFF);  		
		}			
	#elif (VERSION == 0x0119 || VERSION == 0x0219 || VERSION == 0x0128 || VERSION == 0x0130 || VERSION == 0x0228 || VERSION == 0x0230)
		{
			create_F_M(&filter1, &mask1,ICUBCANPROTO_CLASS_POLLING_MOTORCONTROL,0xFF, _board_ID  , ICUBCANPROTO_CLASS_PERIODIC_ANALOGSENSOR, 0xFF, 0xFF);   
			create_F_M(&filter2, &mask2,ICUBCANPROTO_CLASS_POLLING_MOTORCONTROL,0xFF, _board_ID+1, ICUBCANPROTO_CLASS_BOOTLOADER,     0x00, 0xFF);  
		}
	#else
		{
		#if   (JN == 2)
			create_F_M(&filter1, &mask1,ICUBCANPROTO_CLASS_POLLING_MOTORCONTROL,0xFF, _board_ID, ICUBCANPROTO_CLASS_PERIODIC_ANALOGSENSOR, 0xFF, 0xFF);   
			create_F_M(&filter2, &mask2,ICUBCANPROTO_CLASS_BOOTLOADER,  0x00, 0xFF, 	   ICUBCANPROTO_CLASS_BOOTLOADER,  	0x00, 0xFF);  
		#elif (JN == 4)
			create_F_M(&filter1, &mask1,ICUBCANPROTO_CLASS_POLLING_MOTORCONTROL,0xFF, _board_ID,   ICUBCANPROTO_CLASS_BOOTLOADER,   0x00, 0xFF);
			create_F_M(&filter2, &mask2,ICUBCANPROTO_CLASS_POLLING_MOTORCONTROL,0xFF, _board_ID+1, ICUBCANPROTO_CLASS_BOOTLOADER,  	0x00, 0xFF);  
		#endif
		}
	#endif	
