...
#if VERSION == 0x0250 || VERSION == 0x0150
...
void decouple_dutycycle_new_joint_parametric(Int32 *pwm)
{
	...
	*/	
	...
	if (_control_mode[1] == MODE_POSITION)
	{
		...
		tempf =  (float)(_pd[0]) * _param_a10_coeff;
		tempf += (float)(_pd[1]) * _param_a11_coeff; 
		temp32 = (Int32) (tempf);
		pd_out[1] = (Int32) temp32;

		tempf =  (float)(pwm[0]) * _param_a10_coeff;
		tempf += (float)(pwm[1]) * _param_a11_coeff; 
		temp32 = (Int32) (tempf);
	 	pwm_out[1] = (Int32) temp32;				
	}
	...
	*/	  
	...
	if (_control_mode[0] == MODE_POSITION)
	{
		...
		// 		    [-t  t  t]   
   	    ...
   	    tempf =  (float) ( _cpl_pid_prediction[0]) * _param_a20_coeff;
   	    tempf += (float) ( _cpl_pid_prediction[1]) * _param_a21_coeff;
   	    tempf += (float) ( _pd[0]) 				   * _param_a22_coeff;
	    temp32 = (Int32) (tempf);
	    pd_out[0] = temp32;
	    
   	    tempf =  (float) ( _cpl_pid_prediction[0]) * _param_a20_coeff;
   	    tempf += (float) ( _cpl_pid_prediction[1]) * _param_a21_coeff;
   	    tempf += (float) ( pwm[0])				   * _param_a22_coeff;
	    temp32 = (Int32) (tempf);
	    pwm_out[0] = temp32;	    
	}
