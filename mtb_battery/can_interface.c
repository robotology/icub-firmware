...
int CAN1_handleRx (unsigned int board_id)
{
    ...
    while (CAN1_getRxbufferIndex()>0)
    {
        ...
        if ((CANRxBuffer[canRxBufferIndex-1].CAN_messID & 0x700) == 0x700)
        {
        ...
        //  Periodic message, 
        // ID 0x100 (001 0000 0000b) message class = periodic message                
        if ((CANRxBuffer[canRxBufferIndex-1].CAN_messID & 0x700) == 0x100)
		{
    		...
    		switch (CANRxBuffer[canRxBufferIndex-1].CAN_data[0])
			{
				case CAN_TACT_SETUP:
				{
					...
					board_MODE=	CANRxBuffer[canRxBufferIndex-1].CAN_data[1]&0xF;
					...
					if ((CANRxBuffer[canRxBufferIndex-1].CAN_data[2]&0xf0)==0x10)
					{	
						PW_CONTROL= 0x40; // for 256 decim
					}
					if ((CANRxBuffer[canRxBufferIndex-1].CAN_data[2]&0xf0)==0x20)
					{
						PW_CONTROL= 0x140; // for 128 decim
					}
					if ((CANRxBuffer[canRxBufferIndex-1].CAN_data[2]&0xf0)==0x30)
					{
						PW_CONTROL= 0x240; // for 64 decim
					}
					
			//		board_TIMER=	CANRxBuffer[canRxBufferIndex-1].CAN_data[3];
					
					if ((CANRxBuffer[canRxBufferIndex-1].CAN_data[3]&0xF)==0x1)
					{	
						TIMER_VALUE=0x2D00; // for 256 decim
					}
					if ((CANRxBuffer[canRxBufferIndex-1].CAN_data[3]&0xF)==0x2)
					{
						TIMER_VALUE= 0x1700; // for 128 decim
					}
					...
					//	TIMER_VALUE= 0xB40; // for 64 decim
						
					}
					
					if ((CANRxBuffer[canRxBufferIndex-1].CAN_data[3]&0xF0)==0x10)
					{	
						TIMER_VALUE=0x1000; // for 256 decim
					}
					if ((CANRxBuffer[canRxBufferIndex-1].CAN_data[3]&0xF0)==0x20)
					{
						TIMER_VALUE= 0x1000; // for 128 decim
					}
					if ((CANRxBuffer[canRxBufferIndex-1].CAN_data[3]&0xF0)==0x30)
					{
						TIMER_VALUE=(CANRxBuffer[canRxBufferIndex-1].CAN_data[4]) | (CANRxBuffer[canRxBufferIndex-1].CAN_data[5]<<8);
					//	TIMER_VALUE= 0xB40; // for 64 decim
						
					}
		//ConfigAll
						ConValue[0]=(CANRxBuffer[canRxBufferIndex-1].CAN_data[4]) | (CANRxBuffer[canRxBufferIndex-1].CAN_data[5]<<8);
						ConValue[1]=(CANRxBuffer[canRxBufferIndex-1].CAN_data[6]) | (CANRxBuffer[canRxBufferIndex-1].CAN_data[7]<<8);
